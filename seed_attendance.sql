...
SELECT a.id, a.coach_id, '2026-02-10', 'Present', 'Solid session', 'Arlington Drive' FROM athletes a
WHERE a.name IN ('Alice Johnson', 'Bob Smith', 'Charlie Brown', 'Fiona Gallagher')
UNION ALL
SELECT a.id, a.coach_id, '2026-02-10', 'Absent', '-', 'Arlington Drive' FROM athletes a
WHERE a.name IN ('Diana Prince', 'Evan Wright', 'George Bailey')
ON CONFLICT (athlete_id, date) DO NOTHING;

-- Feb 3, 2026
INSERT INTO attendance (athlete_id, coach_id, date, status, result, session_name)
SELECT a.id, a.coach_id, '2026-02-03', 'Present', 'PB on 100m', 'Speed Endurance' FROM athletes a
WHERE a.name IN ('Alice Johnson', 'Diana Prince', 'Fiona Gallagher')
UNION ALL
SELECT a.id, a.coach_id, '2026-02-03', 'Present', '2:53 for 0.45m', 'Speed Endurance' FROM athletes a
WHERE a.name IN ('Bob Smith', 'Charlie Brown')
UNION ALL
SELECT a.id, a.coach_id, '2026-02-03', 'Late', 'Arrived 15m late', 'Speed Endurance' FROM athletes a
WHERE a.name IN ('Evan Wright')
ON CONFLICT (athlete_id, date) DO NOTHING;

-- Jan 27, 2026
INSERT INTO attendance (athlete_id, coach_id, date, status, result, session_name)
SELECT a.id, a.coach_id, '2026-01-27', 'Present', 'Warm up drills', 'Technical Session' FROM athletes a
